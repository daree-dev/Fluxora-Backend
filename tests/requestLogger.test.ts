...
describe('requestLogger middleware', () => {
  ...
  it('logs "request received" on the way in', async () => {
    ...
    const messages = calls.map((c: any) => c[0] as string);
  ...
  it('passes the correlationId to both log calls', async () => {
    ...
    const calls = infoSpy.mock.calls;
    const correlationIds = calls.map((c: any) => c[1] as string);
    expect(correlationIds.every((id: string) => id === clientId)).toBe(true);
  });

  it('"request received" log includes method and path', async () => {
    await request(app).get('/health');
    const receivedCall = infoSpy.mock.calls.find((c: any) => c[0] === 'request received');
  ...
  it('"request completed" log includes statusCode and durationMs', async () => {
    await request(app).get('/health');
    const completedCall = infoSpy.mock.calls.find((c: any) => c[0] === 'request completed');
